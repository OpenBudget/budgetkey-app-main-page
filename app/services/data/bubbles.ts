...
const SQL_FUNC_BUBBLES_DATA = `
  ...
  FROM raw_budget
  WHERE length(code) = 10 AND year = 2018 AND NOT code LIKE '0000%%'
  GROUP BY 1, 2 
  ORDER BY 1, 2
`;
...
const SQL_INCOME_BUBBLES_DATA = `
  SELECT 
    ...
    sum(net_allocated) AS total_amount
  FROM raw_budget
  WHERE length(code) = 10 AND year = 2018 AND code LIKE '0000%%'
  GROUP BY 1, 2 
  HAVING sum(net_allocated) > 0
  ORDER BY 1, 2
`;
...
function deficitChart() {
  return Promise.all([
    fetch_doc('budget/00/2018'),
    fetch_doc('budget/0000/2018')
  ]).then((data: any) => {
    return {
