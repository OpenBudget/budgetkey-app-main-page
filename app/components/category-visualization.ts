...
export class CategoryVisualizationInfoPopup implements OnInit {
  @Input() bubble: any;
...
export class CategoryVisualizationComponent implements OnInit, AfterViewInit {
  ...
  private formatPercents(value: number) {
    return Number(value).toFixed(1) + '%';
  }
  ...
  private renderLegendLabels(container: any, root: any, nodes: any[], margin: number, padding: number) {
    container
      ...
      .enter()
      .append('g')
      .call(createLegendLabels, margin / 2, this.formatPercents)
